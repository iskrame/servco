...
exports.createOrUpdateCollaborator = async function (req, res) {
    try {
        ...
        if (req.body.municipality) collaboratorFields.address.municipality = req.body.municipality;
        if (req.body.addresState) collaboratorFields.address.addresState = req.body.addresState;
        ...
        if (req.body.chronicdiseases) collaboratorFields.emergenciesInformation.chronicdiseases = req.body.chronicdiseases;
        if (req.body.bloodType) collaboratorFields.emergenciesInformation.bloodType = req.body.bloodType;
        if (req.body.allergies) collaboratorFields.emergenciesInformation.allergies = req.body.allergies;
        ...
        if (req.body.accountNumber) collaboratorFields.payWay.accountNumber = req.body.accountNumber;
        if (req.body.cardNumber) collaboratorFields.payWay.cardNumber = req.body.cardNumber;
        ...
        if (req.body.employeeRol) collaboratorFields.employeeRol = req.body.employeeRol;
        ...
        if (req.body.email) {
            await Collaborator.findOne({ email: req.body.email })
                .then(collaborator => {
                    if (collaborator) {
                        if (req.body.id) {
        ...
        if (req.body.clave) {
            await Collaborator.findOne({ clave: req.body.clave })
                .then(collaborator => {
                    if (collaborator) {
                        if (req.body.id) {
                            //it means is updating
                            if (collaborator.id.toString() !== req.body.id) {
                                errorsDataDuplicated.clave = 'La clave de colaborador ya está en uso';
                                bErrors = true;
                            }
                        } else {
                            errorsDataDuplicated.clave = 'La clave de colaborador ya está en uso';
        ...
        if (req.body.id) {
            Collaborator.findByIdAndUpdate(req.body.id, collaboratorFields, { new: true })
                .then(collaborator => {
                    if (collaborator) {
            ...
            new Collaborator(collaboratorFields).save()
        ...
        // } else {
        //     if (req.body.password) {
        //         collaboratorFields.password = req.body.password;
        //         bcrypt.genSalt(10, (err, salt) => {
        //             bcrypt.hash(collaboratorFields.password, salt, (err, hash) => {
        //                 if (err) throw err;
        //                 collaboratorFields.password = hash;
        //                 new Collaborator(collaboratorFields).save()
        //                     .then(collaborator => res.json(collaborator))
...
exports.deleteById = function (req, res) {
    status = false;
    Collaborator.findByIdAndUpdate(req.params.id, { status: status }, { new: true })
        .then(collaborator => {
            if (collaborator) {
                res.json(collaborator)
            } else {
...
exports.createEducation = function (req, res) {
    ...
    if (req.body.additionalInformation) education.additionalInformation = req.body.additionalInformation;
    Collaborator.findById(req.params.id)
        .then(collaborator => {
            if (collaborator) {
